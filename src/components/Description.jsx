...
function Description({ onDescriptionChange, onStateOfProductChange }) {
  ...
  const handleOptionSelect = (option) => {
    ...
    onStateOfProductChange(option); // Passing state to parent
  };
  ...
  return (
    <div className="md:mx-36 ">
      ...
      <div className="col-span-full">
        <label htmlFor="about" className="block text-sm leading-6 text-gray-700 font-bold text-left mt-3">
          ¿En qué estado está?
        </label>
        <div className="mt-2">
          <div>
            <div className={`flex flex-col rounded-md m-1 w-50 h-50 border ${selectedOption === "perfecto" ? "border-orange-300" : ""}`}
            ...
            <div className={`flex flex-col rounded-md m-1 w-50 h-50 border ${selectedOption === "golpeadon" ? "border-orange-300" : ""}`}
              onClick={() => handleOptionSelect("golpeadon")}>
              <label className="text-gray-700 p-2 text-left" htmlFor="StateofProduct">
                Golpeadón, tiene algunos raspones o marcas de uso
              </label>
            </div>
            <div className={`flex flex-col rounded-md m-1 w-50 h-50 border ${selectedOption === "vivido" ? "border-orange-300" : ""}`}
